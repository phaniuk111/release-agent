...
- **Remove / unstage:** before the day is locked, "remove `<image>` from today's release" (`remove_from_release`) pulls an image back out — it reverts that image to the tag currently on PRD, or drops it if it was new, so it no longer ships today. Reversible (promote again). Refused once the UAT → PRD PR is raised (points you to that PR to amend/close).
