...
To serve several apps under one domain (e.g. `app-acme-uat.com`) by path, give each a
...
helm upgrade --install rc helm/release-copilot -n release \
  ...
  --set virtualService.hosts="{app-acme-uat.com}" \
  --set virtualService.gateways="{istio-ingress/asm-ingressgateway}" \
...
App is then served at **`https://app-acme-uat.com/release-copilot/`**. The VirtualService:
...
- The shared gateway must serve the host (`app-acme-uat.com`, or `*`).
