...
Every screen is imported from its own module (`./forms/queue_table.js`, …);
there is no barrel index re-exporting them.
...
- A labelled field is `labeledField(grid, spec)` from `forms/common.js` (the
  control alone: `fieldControl(spec)`) — never a new copy of the class string.
- A name goes on `window` only if an `onclick=` in the served HTML calls it.
...
2. Keep the words: `queueDestination(q)`, `buildSummary(q)`, `controlsSummary(q)`,
   `timeAgo(...)`, `shortName(...)`.
