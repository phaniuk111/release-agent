let threadId = localStorage.getItem('thread_id') || 'fastapi-' + Math.random().toString(36).slice(2, 10);
        ...
        // ---- Insights drawer: collapsible report sections -----------------------
        // A section registry, so adding the next report (quality gates, deploy
        // history, ...) is one entry with a render() — no new layout code. Each
        // section is a native <details> accordion; content renders lazily on
