...
// Collects the full release_details payload: CHG fields, artifact list,
...
export async function showReleaseForm() {
    ...
    wrap.innerHTML =
        '<div class="mb-1 font-semibold flex items-center gap-2 text-emerald-300">' +
        '<i class="fa-solid fa-box-open"></i> Create release</div>' +
    ...
    const submit = document.createElement('button');
    submit.className = 'bg-emerald-600 hover:bg-emerald-500 px-4 py-1.5 rounded-lg text-sm font-medium';
    submit.textContent = 'Create release';
    const err = document.createElement('span');
    err.className = 'text-[11px] text-red-400';
    submit.addEventListener('click', () => {
        ...
        const payload = {
            ...
            prl1_only: Array.from(flags.querySelectorAll('input[data-prl1]:checked')).map(c => c.dataset.prl1),
            df_images: Array.from(flags.querySelectorAll('input[data-df]:checked')).map(c => c.dataset.df),
            artefact: artifacts,
        };
