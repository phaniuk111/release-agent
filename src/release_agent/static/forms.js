...
// Same wording as the Backstage queue tab, so both UIs describe a row alike.
export function queueDestination(q) {
    const picked = String(q.target_envs || '').split(',').map(e => e.trim());
...
export async function showQueueTable() {
    const _ph = _opening('the release queue');
    const chat = document.getElementById('chat');
    const wrap = document.createElement('div');
...
async function _renderQueueTable(wrap, flash) {
    const ctx = await _ctx('/api/release-queue', { queue: [] });
    ...
    const note = _ctxNote(ctx, 'the queue'); if (note) wrap.appendChild(note);
        ...
        items.forEach((q, i) => {
            ...
            html += '<tr class="border-b border-slate-800 align-top" data-row="' + i + '">' +
                ...
                '<td class="px-2 py-1.5 text-slate-400 whitespace-nowrap" title="' +
                    ...
                    esc((q.requested_by || '—').split('@')[0]) +
                    '<div class="text-slate-600">' + _ago(q.requested_at) + '</div></td>' +
...
function _confirmRemove(wrap, scroller, q, index) {
    ...
    go.addEventListener('click', async () => {
        ...
        let res = null;
        try {
            const r = await fetch(API_BASE + '/api/release-queue/withdraw', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ artifact_name: q.artifact_name,
