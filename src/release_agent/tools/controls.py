...
def verify_image_tag_build(
    ...
    def _inspect(run):
        ...
        try:
            for job in run.jobs():
                # The tag may be generated by a whole JOB rather than a step —
                # see run_built_tags; its steps are then named after the actions
                # they run, and a step-only match would find nothing.
                if tag_step is None and tag_name_matches(tag_generation_step, job.name or ""):
                    tag_step = {"job": job.name, "job_id": job.id, "number": None,
                                "name": job.name, "status": getattr(job, "status", None),
                ...
                for step in getattr(job, "steps", None) or []:
                    ...
                    if tag_name_matches(tag_generation_step, name) and (
                            tag_step is None or tag_step.get("number") is None):
                        tag_step = rec
