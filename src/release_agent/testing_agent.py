...
Environment:
    ...
    - OPENAI_API_KEY
    - RELEASE_AGENT_TARGET_REPO (manifest repo)
...
from .tools.gh_tools import (
    GH_TOOLS as COPILOT_TOOLS,  # we can reuse some
    find_prs,
    get_pr_comments,
    get_pr_details,
...
class InvokeCopilotInput(BaseModel):
    message: str = Field(..., description="Message to send to the release copilot (e.g. 'promote payments-api:2.0.99-test')")
    thread_id: str = Field(
        default="",
...
class LocatePRInput(BaseModel):
    search_term: str = Field(..., description="Search term (image:tag or similar) to find the PR created by the workflow")
...
class RunFullScenarioInput(BaseModel):
    image_tags: str = Field(..., description="Comma-separated image:tag to test, e.g. 'payments-api:2.0.99-test'")
...
def simulate_confirmation(token: str, thread_id: str) -> str:
    ...
    copilot = _get_copilot()
    result = copilot.invoke(
        {"messages": [HumanMessage(content=token)]},
    ...
    return json.dumps({
        "thread_id": thread_id,
        "response_after_confirmation": content[:2000]
    }, indent=2)
...
def run_full_e2e_scenario(image_tags: str, scenario_name: str = "full-release-e2e") -> str:
    ...
    if token:
        ...
        step2 = TestStep(name="simulate_confirmation", status="skipped",
    ...
    report.overall_status = "passed" if passed == total and total > 0 else "partial" if passed > 0 else "failed"
    report.summary = f"{passed}/{total} steps passed for scenario {scenario_name} (images={image_tags})"
...
def build_tester_graph(checkpointer=None):
    ...
    graph.add_conditional_edges("llm", lambda s: "tools" if getattr(s.messages[-1], "tool_calls", None) else END)
...
def run_end_to_end_test(image_tags: str = "payments-api:2.0.99-test,orders-api:v9.9.9-test",
                        scenario_name: str = "full-release-e2e") -> TestReport:
    """
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run the Release Copilot testing subagent end-to-end")
    parser.add_argument("--image-tags", default="payments-api:2.0.99-test", help="image:tag pairs to test")
